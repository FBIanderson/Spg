...
def main(slice_id):
    ...
    for node in tqdm.tqdm(all_func_node):
        ...
        cfg_path = os.path.join("/home/zheng/Desktop/qemucfg/"+str(slice_id)+"/cfg_db", testID,store_file_name)
        cfg = pickle.load(open(cfg_path+'/cfg'))
        recordtemp = []
        print 1
        for i in tqdm.tqdm(range(cfg.ecount())):
            d = dict()
            d['sid'] = str(cfg.vs[cfg.es[i].source]['name'])
            if cfg.vs[cfg.es[i].source]['location'] != None:
                d['spro'] = {'type': str(cfg.vs[cfg.es[i].source]['type']), 'code': str(cfg.vs[cfg.es[i].source]['code']),
                         'lineNo': str(cfg.vs[cfg.es[i].source]['location'].split(':')[0])}
            elif cfg.vs[cfg.es[i].source]['location'] == None:
                d['spro'] = {'type': str(cfg.vs[cfg.es[i].source]['type']), 'code': str(cfg.vs[cfg.es[i].source]['code']),
                         'lineNo': 'None'}
            d['eid'] = str(cfg.vs[cfg.es[i].target]['name'])
            if cfg.vs[cfg.es[i].target]['location'] != None:
                d['epro'] = {'type': str(cfg.vs[cfg.es[i].target]['type']),
                             'code': str(cfg.vs[cfg.es[i].target]['code']),
                             'lineNo': str(cfg.vs[cfg.es[i].target]['location'].split(':')[0])}
            elif cfg.vs[cfg.es[i].target]['location'] == None:
                d['epro'] = {'type': str(cfg.vs[cfg.es[i].target]['type']), 'code': str(cfg.vs[cfg.es[i].target]['code']),
                         'lineNo': 'None'}
            d['label'] = str(cfg.es[i]['label'])
            recordtemp.append(d)
        print 1
        startline_path = cfg.vs[1]['filepath']
    ...
    store_record_path = '/home/zheng/Desktop/QEMU_CFG_record/'+str(slice_id)+'/JOERNrecord.txt'
    store_real_path = '/home/zheng/Desktop/QEMU_CFG_record/'+str(slice_id)+'/JOERNreal-mvd.txt'
    for r in record:
