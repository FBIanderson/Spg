...
def modifyDataEdgeVal(pdg):
    for edge in pdg.es:
        if edge['var'] is None: # Control Edge
            continue
...
def modifyStmtNode(pdg):
    ...
    while j < pdg.vcount():
        if pdg.vs[j]['location'] is not None:
...
def completeDeclStmtOfPDG(pdg, dict_use, dict_def, dict_if2cfgnode, dict_cfgnode2if):
    ...
    dict_declnode2val = {}
    for node in pdg.vs:
        if (node['type'] == 'IdentifierDeclStatement' or node['type'] == 'Parameter' or node['type'] == 'Statement') \
                and node['code'].find(' = ') == -1:  # find not init node
...
def main(slice_id):
    ...
    for node in tqdm.tqdm(all_func_node):
        ...
        cfg_path = os.path.join("/home/anderson/Desktop/locator_cfg/" + str(slice_id) + "/cfg_db", testID,
                                store_file_name)

        fin = open(os.path.join(cfg_path, 'dict_if2cfgnode'))
        dict_if2cfgnode = pickle.load(fin)
        fin.close()

        fin = open(os.path.join(cfg_path, 'dict_cfgnode2if'))
        dict_cfgnode2if = pickle.load(fin)
        fin.close()

        fin = open(os.path.join(cfg_path, 'cfg'))
        cfg = pickle.load(fin)
        fin.close()
        ...
        while i < opt_pdg_1.vcount():
            if opt_pdg_1.vs[i]['type'] == 'Statement' and opt_pdg_1.vs[i]['name'] not in cfg.vs['name']:
                for n in cfg.vs:
                    if opt_pdg_1.vs[i]['code'] == n['code'] and \
                            int(opt_pdg_1.vs[i]['location'].split(':')[0]) == int(n['location'].split(':')[0]):
